...
namespace Matrix.CheckServer
{
    ...
    class ApiConnector : IConnector
    {
        ...
        {
            string operationAction = ConfigurationManager.AppSettings["webServer"];
            Process.Start(operationAction);
            log.Debug(string.Format("restart web"));
        }
        private void OnDataReceived(string obj)
        {
            try
            {
                ...
                {
                    countForRestart = 0;
                    log.Debug("пришел пинг от сервера");
                    isPingOk = true;
                    return;
                }
                #endregion
