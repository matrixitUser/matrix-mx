...
namespace Matrix.Poll.Driver.Scylar
{
    internal class VariableDataBlock
    {
                ...
                case 13:
                    {
                        ...
                        {
                            //Driver.Log(string.Format("TimePoint=[{0}]", string.Join(",", rowData.Select(b => b.ToString("X2")))));
        ...
        private DateTime ParseDateTime(byte[] data)
        {
            ...
            //Driver.Log(string.Format("year=2000 + ({0:X2}&F0 >> 1) | ({1:X2}&E0 >> 5)=2000 + ({2:X2} >> 1) | ({3:X2} >> 5) = 2000 + {4:X2}|{5:X2} = {6:X2}", data[3], data[2], (data[3] & 0xF0), (data[2] & 0xE0), (data[3] & 0xF0) >> 1, (data[2] & 0xE0) >> 5, year));
