...
namespace Matrix.Poll.Driver.Scylar
{
	public static class Parser
	{
		...
		private static Tuple<DateTime, IEnumerable<dynamic>> ConvertToData(IEnumerable<VariableDataBlock> collection, ArchiveType archiveType)
		{
			...
			foreach (var dataBlock in list)
			{
                ...
                dynamic record;
                if(archiveType == ArchiveType.Hourly)
                {
                    record = DriverHelper.Instance().MakeHourRecord(string.Format("{0}{1}{2}", uglyParameter.ParameterType, uglyParameter.Channel, uglyParameter.CalculationType), value, measuringUnitType.GetString(), recordDateTime);
                }
                else if (archiveType == ArchiveType.Daily)
                {
                    record = DriverHelper.Instance().MakeDayRecord(string.Format("{0}{1}{2}", uglyParameter.ParameterType, uglyParameter.Channel, uglyParameter.CalculationType), value, measuringUnitType.GetString(), recordDateTime);
                }
                else
                {
                    record = DriverHelper.Instance().MakeCurrentRecord(string.Format("{0}{1}{2}", uglyParameter.ParameterType, uglyParameter.Channel, uglyParameter.CalculationType), value, measuringUnitType.GetString(), recordDateTime);
                }
                recordList.Add(record);// new Data(string.Format("{0}{1}{2}", uglyParameter.ParameterType, uglyParameter.Channel, uglyParameter.CalculationType), measuringUnitType, recordDateTime, value));
			}

			return new Tuple<DateTime, IEnumerable<dynamic>>(recordDateTime, recordList);
		}
