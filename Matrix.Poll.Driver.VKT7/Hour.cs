...
namespace Matrix.Poll.Driver.VKT7
{
    ...
    {
        dynamic GetHours(DateTime start, DateTime end, DateTime current, dynamic properties, IEnumerable<int> channels)
        {
            ...
            var filterElements = FilterElements((List<dynamic>)elements.ActiveElements, ValueType.Hour, channels);
            ...
            for (var date = start.Date.AddHours(start.Hour); date <= end; date = date.AddHours(1))
            {
                ...
                var data = ParseReadDataResponse(Send(MakeReadArchiveRequest()), date, properties.Fracs, properties.Units, filterElements, ValueType.Hour);//elements.ActiveElements, 
