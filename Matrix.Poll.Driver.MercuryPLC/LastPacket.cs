    ...
    internal class ParameterValue
    {
        ...
        public static ParameterValue ParseData(byte type, byte[] data)
        {
            switch(type)
            {
                ...
                case 0x42:
                case 0x43:
                    return new ParameterValueDay(string.Format(EE_TARIFF_FORMAT, type - 0x40 + 1), ParseDecValueExt(data), EE_UNIT_KWTH);

                case 0x44:
                    return new ParameterValueHalfHour(EE_TARIFF_ALL, ParseDecValueHH(data), EE_UNIT_KWTH, data[0]);
                case 0x4F:
                    return new ParameterValueDay(EE_TARIFF_ALL, ParseDecValueExt(data), EE_UNIT_KWTH);
            }
            return null;
        }
