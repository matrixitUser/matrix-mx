...
namespace Matrix.Poll.Driver.MercuryPLC
{
    ...
    internal static class Crc24
    {
        //===================================================================/
        private const UInt32 CRC24_INIT = 0x00b704ce;
        private const UInt32 CRC24_POLY = 0x01864cfb;
        //===================================================================/
        private static readonly UInt32[] CRC24tab = {
        ...
        public static UInt32 Compute(byte[] octets, int startIndex, int length)
        {
            int maxLen = octets.Length - startIndex;
            int len = length > maxLen ? maxLen : length;
            ...
            while (len-- > 0)
            {
                ...
                temp >>= 8;
                temp >>= 8;
                temp ^= octets[startIndex++];
                arg = (int)(temp & 0x000000FF);
                crc <<= 8;
                crc ^= CRC24tab[arg];
            }
            crc &= (UInt32)0x00ffffffL;
            return crc;
        }

        public static UInt32 Compute(byte[] octets, int startIndex = 0)
        {
            return Compute(octets, startIndex, octets.Length);
        }
    }
}
